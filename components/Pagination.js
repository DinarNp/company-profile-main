...
const Pagination = ({ section, currentPage, totalPages, slug }) => {
    ...
    <>
      {totalPages > 1 && (
        <nav className="mb-4 mt-10 flex justify-center space-x-[10px]" aria-label="Pagination">
          ...
          {hasPrevPage ? (
            <Link
              href={
                indexPageLink
                  ? `${section ? '/' + section : '/'}`
                  : `${section ? '/' + section : ''}/${slug}/${currentPage - 1}`
              }
          ...
          {pageList.map((pagination, i) => (
            <React.Fragment key={`page-${i}`}>
              {pagination === currentPage ? (
                <span aria-current="page" className={`rounded-md bg-primary px-4 py-2 text-white`}>
                  {pagination}
                </span>
              ) : (
                <Link
                  ...
                  href={
                    i === 0
                      ? `${section ? '/' + section : ''}/${slug}/${pagination}`
                      : `${section ? '/' + section : ''}/${slug}/${pagination}`
                  }
                  passHref
                >
          ...
          {hasNextPage ? (
            <Link
              href={`${section ? '/' + section : ''}/${slug}/${currentPage + 1}`}
              className="inline-flex w-[42px] justify-center rounded-md bg-theme-light px-2 py-2 text-dark hover:bg-primary hover:text-white"
            >
